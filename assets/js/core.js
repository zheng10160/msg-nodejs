    ...
    $(function () {
        ...
        socket.on('is_check', function (u) {
            ...
            if(u && u.userid && u.username){
                ...
                info = {"uid": u.userid, "username": u.username,"avatar":u.avatar,"sex":u.sex,"toUserInfo":toUserInfo};
            ...
            socket.on('room-message', function (msg) {
                ...
                if (msg.type == 'private') {
                    ...
                    socket.emit('del-state',msg.other_userinfo);//时时聊天时需要清除占存取
                    ...
                    $(document).ready(function () {
                        ...
                        socket.emit('del-state',msg.other_userinfo);
             ...
             /********独立处理从地图上直接切换的时候需要展示对话页面 start*********/

             socket.on('to-chat-user', function (result) {

                 //将当前需要建立聊天的用户值付于全局

                 $("#toUser input[name='to_username']").val(result.toUsername);

                 $("#toUser input[name='to_userid']").val(result.toUserid);

                 $("#toUser input[name='to_avatar']").val(result.toUserAvatar);

                 $("#toUser input[name='to_sex']").val(result.toUserSex);



                 $(this).parent().parent().addClass('active_li').siblings().removeClass('active_li');

                 socket.emit('to_user', {toUsername: result.toUsername,toUserid:result.toUserid,toUserAvatar:result.toUserAvatar,toUserSex:result.toUserSex});

             })



            /********独立处理从地图上直接切换的时候需要展示对话页面 end*********/
                ...
                if(msg_log.msg_read_log) {
                    ...
                    for(var i in read_v){
                        ...
                        if(data.userid == uid){
                            ...
                            str += '<div class="chat-message"><pre style="white-space: pre-wrap;">'+data.content+'<div class="chehui">撤回</div></pre></div>';
                          ...
                          /*  str += '<p class="weidu">未读</p>';*/
                            ...
                            str += '<div class="chat-message"><pre style="white-space: pre-wrap;"> '+data.content+'</pre></div>';
